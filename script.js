...
function move(){
    ...
    if(snakeBody[0].classList.contains('snakeBody')){
        ...
        clearInterval(intervalMoveSnake);
        snakeBody[0].style.background = 'url(./pic/screamCat.png) center no-repeat';
...
let intervalMoveSnake = setInterval(move,300);

window.addEventListener('keydown',(event)=>{
    let keyPrssed = event.code;
    
    if(keyPrssed == 'ArrowRight' && direction !='left' ){
        direction = 'right';
    } else if(keyPrssed == 'ArrowLeft'  && direction !='right') {
        direction = 'left';
    } else if(keyPrssed == 'ArrowUp'  && direction !='down') {
        direction = 'up';
    } else if(keyPrssed == 'ArrowDown' && direction !='up') {
            direction = 'down';
    }
})
